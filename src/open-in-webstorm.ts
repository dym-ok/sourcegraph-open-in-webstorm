...
interface Settings {
    'openInWebstorm.basePath'?: string
}

function getOpenUrl(textDocumentUri: URL): URL {
    const basePath = sourcegraph.configuration.get<Settings>().value['openInWebstorm.basePath']
    const learnMorePath = new URL('/extensions/sourcegraph/open-in-webstorm', sourcegraph.internal.sourcegraphURL.href)
        .href
    const userSettingsPath = new URL('/user/settings', sourcegraph.internal.sourcegraphURL.href).href

    if (typeof basePath !== 'string') {
        throw new TypeError(
            `Add \`openInWebstorm.basePath\` to your [user settings](${userSettingsPath}) to open files in the editor.`+
            `[Learn more](${learnMorePath})`
        )
    }
    if (!path.isAbsolute(basePath)) {
        throw new Error(
            `\`openInWebstorm.basePath\` value \`${basePath}\` is not an absolute path.`+
          `Please correct the error in your [user settings](${userSettingsPath}).`
        )
    }
    ...
    const openUrl = new URL('webstorm://open?file=' + absolutePath)
...
export function activate(context: sourcegraph.ExtensionContext): void {
    context.subscriptions.add(
        sourcegraph.commands.registerCommand('openInWebstorm.open.file', async (uri?: string) => {
            if (!uri) {
